...
do
    ...
    function a.x()
        ...
        -- === Section system + safe NewTab override ===
        -- Put this inside the same scope where `l`, `i`, `j`, `h`, `a` exist
        -- (i.e. inside the a.x() function near other UI constructors).

        -- make sure we keep the original NewTab
        if not l._OldNewTab then
            l._OldNewTab = l.NewTab
        end

        -- override NewTab safely (wraps original and injects :Section())
        l.NewTab = function(...)
            local type_, tab = l._OldNewTab(...)
            -- attach Section() factory to the tab
            function tab:Section(props)
                return l.NewSection(self, props or {})
            end
            return type_, tab
        end

        -- Section constructor
        function l.NewSection(parentTab, props)
            props = props or {}
            local section = {
                __type = "Section",
            ...
            -- header (use existing 'p' factory)
            section.UIElements.Header = a.load'p'{
                Title = section.Title,
                Parent = parentTab.UIElements.ContainerFrame, -- same container as tab elements
                TextOffset = 0,
                Hover = true,
            }
            ...
            -- content frame (will contain children elements)
            section.UIElements.Content = i("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.new(1, 0, 0, 0), -- collapsed initially
                AutomaticSize = Enum.AutomaticSize.Y,
                Parent = parentTab.UIElements.ContainerFrame,
                ClipsDescendants = true,
            }, {
            ...
            -- internal helper to re-parent created element UI into the section
            local function reparentElementToSection(elem)
                if not elem or not elem.UIElements then return end
                -- most elements either expose UIElements.Dropdown or UIElements.Main (or both)
                if elem.UIElements.Dropdown then
                    elem.UIElements.Dropdown.Parent = section.UIElements.Content
                end
                if elem.UIElements.Main then
                    elem.UIElements.Main.Parent = section.UIElements.Content
                end
                table.insert(section.Children, elem)
            end

            -- Toggle: overloaded
            -- - if called with table -> create a Toggle element in this section (sec:Toggle{...})
            -- - if called with boolean or nil -> open/close the section (sec:Toggle() toggles)
            function section:Toggle(arg)
                if type(arg) == "table" then
                    -- add a Toggle element into this section
                    arg.Parent = section.UIElements.Content
                    -- call the tab's existing Toggle method (it sets Window/WindUI correctly)
                    if type(section.ParentTab.Toggle) == "function" then
                        local ok, elem = pcall(function() return section.ParentTab:Toggle(arg) end)
                        if ok and elem then
                            reparentElementToSection(elem)
                            return elem
                        end
                    end
                    -- fallback: try constructor directly
                    local toggleMod = a.load't'
                    if toggleMod and type(toggleMod.New) == "function" then
                        local _, elem = toggleMod:New(arg)
                        if elem then
                            reparentElementToSection(elem)
                            return elem
                        end
                    ...
                    -- act as open/close
                    local open = arg
                    if open == nil then open = not self.Opened end
                    self.Opened = open
                    if self.Opened then
                        if contentLayout then
                            j(self.UIElements.Content, 0.18, {
                                Size = UDim2.new(1, 0, 0, contentLayout.AbsoluteContentSize.Y)
                            }):Play()
                        else
                            self.UIElements.Content.Size = UDim2.new(1,0,0,0)
                        end
                        arrow.Rotation = 0
                    else
                        j(self.UIElements.Content, 0.18, {
            ...
            -- generic element helpers (Slider, Dropdown, Button, Paragraph, Textbox etc.)
            local elementNames = {"Slider","Dropdown","Button","Textbox","Paragraph","Keybind","Input","Colorpicker","Code"}
            for _, name in ipairs(elementNames) do
                section[name] = function(self, props)
                    props = props or {}
                    props.Parent = self.UIElements.Content
                    if type(self.ParentTab[name]) == "function" then
                        local ok, elem = pcall(function() return self.ParentTab[name](self.ParentTab, props) end)
                        if ok and elem then
                            reparentElementToSection(elem)
                            return elem
                        end
                    end
                    -- fallback: try module constructor directly (best-effort)
                    local modMap = {
                        Slider = 'u', Dropdown = 'x', Button = 'q', Textbox = 'e',
                        Paragraph = 'C', Keybind = 'v', Input = 'w', Colorpicker = 'B', Code = 'A'
                    }
                    local modId = modMap[name]
                    if modId then
                        local mod = a.load[modId]
                        if mod and type(mod.New) == "function" then
                            local _, elem = mod:New(props)
                            if elem then
                                reparentElementToSection(elem)
                                return elem
                            end
            ...
            if section.DefaultOpen then
                -- delay one frame so AbsoluteContentSize calculates properly
                task.spawn(function()
                    task.wait()
                    section:Toggle(true)
                end)
